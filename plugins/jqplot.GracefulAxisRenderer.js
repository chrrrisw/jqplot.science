...
(function($) {
    ...
    $.jqplot.GracefulAxisRenderer.prototype.createTicks = function(plot) {
        ...
        var ticks = generate_ticks({
            min: min,
            max: max
        }, transform, width, height, name, fontSize);
        this.ticks = ticks;
        ...
        while (tick < max) {
            if (Math.abs(tick) < 1e-13) {
                ...
                if (numdigits < -2) {
                    ...
                    var fixeddigits;
                    if (sigdigits < numdigits) {
                        fixeddigits = 0;
                    } else {
                        fixeddigits = sigdigits - numdigits + 1;
                    }
    ...
    function nextLogTick(val, round) {
        ...
        return {
            value: newf * magnitude,
            label: newf.toFixed() + 'e' + expv.toFixed()
        };
    ...
    function generate_ticks(ticklimits, transform) {
        ...
        if (transform == 'lin') {
            ...
            if (scalespan >= 3) {
                ...
                ticks = generate12510ticks(min, max);
            }
        } else {
